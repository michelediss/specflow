...
| `npm run dev:guard -- --mode=architect` | Ensures there are no staged/working changes in `app/src/` or `app/tests/`. Fails otherwise. | Architect mode. Use it before committing or as part of CI on `arch/*` branches. |
...
- (Alternative for dev tasks) Create a branch from a task: `npm run dev:branch -- --task=T-05`
