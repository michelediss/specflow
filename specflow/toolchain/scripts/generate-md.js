...
/**
 * Requirements view mirrors input/REQUISITI.json to let humans scan the spec without touching JSON.
 */
async function generateRequirements() {
  const data = await readJson("input/REQUISITI.json");
  ...
  await writeText("input/REQUISITI.md", lines.join("\n").trim() + "\n");
}
...
async function generateConstraints() {
  const data = await readJson("input/VINCOLI.json");
  ...
  await writeText("input/VINCOLI.md", lines.join("\n").trim() + "\n");
}
...
async function generateStack() {
  const data = await readJson("input/STACK.json");
  ...
  await writeText("input/STACK.md", lines.join("\n").trim() + "\n");
}
...
async function generateSpec() {
  ...
  await writeText("architect/spec/SPEC.md", lines.join("\n").trim() + "\n");
}
...
async function generateTasks() {
  ...
  await writeText("architect/tasks/TASKS.md", lines.join("\n").trim() + "\n");
}
...
async function generateDecisions() {
  const decisions = await readJson("architect/decisions/DECISIONS.json");
  ...
  await writeText("architect/decisions/DECISIONS.md", lines.join("\n").trim() + "\n");
}
